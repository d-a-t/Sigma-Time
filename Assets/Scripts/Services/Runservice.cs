...
public sealed class Runservice : Singleton {
	...
	public void Start() {
		...
		IEnumerator<float> _UpdateLoop() {
			while (Application.isPlaying) {
				UpdateEither(_HeartbeatFuncList, (Time.realtimeSinceStartup - PrevTime_Heartbeat) * Global.Environment.TimeScale);
				PrevTime_Heartbeat = Time.realtimeSinceStartup;
				yield return Timing.WaitForOneFrame;
			}
			yield break;
		}
		CoroutineHandle heartBeat_Coroutine = Timing.RunCoroutine(_UpdateLoop());
		Maid.GiveTask(new Action(delegate () {
			Timing.KillCoroutines(heartBeat_Coroutine);
		}));

		Maid.GiveTask(Runservice.BindToUpdate(Global.RunservicePriority.Heartbeat.First, (float dt) => {
